...
import { Choice, Feedback, Importance, LevelChoice, LevelValue, useAction, useUnsaved } from "@/grades/components";
import { dateLabel, goalFields, goalKey, request } from "@/grades/data";
import type { Assessment } from "@/grades/types";
import { GradeProfileChart } from "./grade-profile-chart";
    ...
    <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="min-w-0 space-y-5">{assessment.categories.map(category => <section key={category.categoryId}>
        ...
        {assessment.scores.filter(s => s.categoryId === category.categoryId).map(score => <div key={score.skillId} className="flex flex-wrap items-center justify-between gap-3 border-b px-3 py-3">
          ...
          {final ? <LevelValue value={score.currentLevel} /> : <LevelChoice label={score.skillName} value={values[score.skillId] ?? score.currentLevel} disabled={action.busy} onChange={level => setValues(old => ({ ...old, [score.skillId]: level }))} />}
        </div>)}
      </section>)}
      ...
      <aside className="min-w-0 space-y-5">
        ...
        <GradeProfileChart categories={assessment.categories} withTarget={assessment.progress !== null} />
