...
Исполнитель завершает свою часть проверенным commit/push и PR. Только назначенный интегратор объединяет пакет и запускает Deploy Stage. Координатор сверяет зависимости и доказательства готовности; роль координатора сама по себе не разрешает deploy. Пользовательское «не деплоить» действует независимо от роли.

Все команды и пути к docs/frontend ниже выполнять относительно выбранного clean worktree, а не historical `D:\HRBot\hr_bot`. Абсолютные примеры не являются разрешением менять rescue checkout.

### Координация пакета

- Product читает релевантные соседние задачи доступными инструментами, проверяет актуальные PR/SHA и зависимости; не просит пользователя пересылать уже доступные отчеты.
- Сообщения/постановку задач другим чатам отправлять только после явного разрешения пользователя. Разрешение может быть выдано один раз на пакет; новые задачи вне пакета в него не входят.
- Перед постановкой проверить, не выполняется ли работа уже. Не перебивать активного исполнителя дублирующей задачей.
- Не вести постоянный polling; читать краткие результаты при следующем рабочем шаге. Фоновый мониторинг требует отдельно настроенной автоматизации.
- В отчете всегда три независимых состояния: «Код проверен» (SHA + проверки/review), «Выложено» (deployed SHA + workflow), «Сценарий проверен» (какое действие, где и результат). Допустимо «не проверено» или «по отчету исполнителя»; отсутствие доказательства не равно успеху.
- Сборка/HTTP 200 не подтверждают действие в браузере или доставку Telegram. Отложенную пользователем приемку явно оставлять открытой.
- Review выбирать по риску: DB для схемы/хранения/identity consistency; Docs для контрактов; Security для границ доступа. После исправления повторно проверять измененный риск, не повторять весь аудит без причины.
- Обязательные CI/checks сохраняются. Дополнительные проверки не повторять на том же SHA без нового изменения, ошибки или неустраненного риска. После интеграции проверить объединенный ref.
- Пользователь решает продуктовые вопросы и принимает результат. Рутинные чтение, диагностика, исправления в feature-ветке и подготовка PR выполняются в рамках согласованной задачи.
...
3. Исполнителю сделать commit/push, подготовить PR и передать координатору SHA, зависимости и результаты проверок.
4. Интегратору объединить согласованный пакет в `stage` или согласованный integration ref и выполнить GitHub Actions `Deploy Stage`.
5. Интегратору выполнить stage smoke и добавить запись в `docs/stage-change-log.md` своего worktree.
6. Отдельно зафиксировать пользовательскую приемку. До нее писать «выложено, сценарий не проверен», а не «полностью готово».
...
- push ветки в GitHub;
- исполнителю остановиться на feature-ветке и передать branch/commit/checks интегратору;
...
Проверено 2026-09-11 по `origin/main` и `origin/stage`: workflow имеет только `workflow_dispatch`, default `ref=stage`; автоматического запуска после CI на main нет. Технически input допускает произвольный ref, поэтому ограничение на интегрированный пакет обеспечивает интегратор. Перед deploy проверять именно выбранную версию workflow: dirty-check в main и stage может различаться.
