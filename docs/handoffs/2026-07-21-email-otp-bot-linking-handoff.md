...
- На базовой OTP-ветке unknown users больше не регистрировались автоматически, но это решение уже пересмотрено в следующем product-slice: controlled `/start` для нераспознанного пользователя снова трактуется как новый кандидат.
