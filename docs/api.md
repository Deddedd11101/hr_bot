...
| `POST` | `/api/flows/workspace/steps/{step_id}` | Обновить один workspace node. | JSON fields: `title`, `text`, `response_type`, `button_options`, `confirm_choice`, `send_mode`, `send_time`, `target_field`, `launch_scenario_key`, `return_to_step_key`, `is_terminal`, `send_employee_card`, `attachment_document_item_id`, `notify_on_send_text`, `notify_on_send_recipient_ids`, `notify_on_send_recipient_scope`, `step_send_notifications[]`, `button_notifications[]`; `target_field` допускает `test_task_result` и `test_task_explanation` для основного ответа и пояснения; text fields хранятся как `TEXT`, runtime рендерит TelegramSafeHTML; notification recipients: `hr`, `manager`, `mentor_adaptation`, `mentor_ipr` | `{ message, payload, step_id }` | Сохраняет шаг и уведомления. `confirm_choice` для buttons/branching включает подтверждение; `is_terminal` завершает leaf после отправки/ответа, container после выбранной цепочки. Attachment library имеет приоритет над upload, legacy attachment_path поддерживает HTTP(S). Несовместимый target/response возвращает 422 без сохранения: resume/candidate_file только file, test_task_result/test_task_explanation file или text. Recipients нормализуются в role-only contract. | `401`, `404`, `422` |
