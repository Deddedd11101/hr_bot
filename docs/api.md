...
| `DELETE` | `/api/employees/{employee_id}` | Удалить карточку сотрудника или кандидата. | Path: `employee_id` | `{ "redirect_url": "/employees" | "/candidates" }` | Удаляет employee row, messenger identity rows, все context progress удаляемой карточки, незавершенный recipient progress, files, offer links и employee storage directory | `401`, `404` |
