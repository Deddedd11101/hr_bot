...
| `POST` | `/api/settings/hr/telegram-link` | Выпустить одноразовую ссылку подтверждения HR Telegram. | Нет | `start_parameter`, optional `deep_link`, `expires_at`, settings workspace | Сохраняет только hash токена и TTL в `hr_settings`; numeric Telegram ID появится после открытия ссылки владельцем | `401` |
