...
- Общий `DELETE /api/employees/{employee_id}` возвращает `409`, если у карточки есть
  финальная оценка грейда. Без финальных оценок вместе с карточкой удаляются профиль,
  черновики и значения Grade. То же ограничение действует в classic delete flow.
...
| `GET` | `/employees/{employee_id}/files/{file_id}/download` | Скачать stored inbound/outbound file из карточки. | Path: `employee_id`, `file_id` | File response | Возвращает файл из `EmployeeFile.stored_path` с `EmployeeFile.mime_type`; покрывает документы, фото и видео, включая Telegram `video` / `video_note`, сохраненные как inbound files | `401`, `303` если файл не найден |
| `DELETE` | `/api/employees/{employee_id}` | Удалить карточку сотрудника или кандидата. | Path: `employee_id` | `{ "redirect_url": "/employees" | "/candidates" }` | Удаляет employee row, messenger identity rows, все context progress удаляемой карточки, незавершенный recipient progress, files, offer links и employee storage directory; Grade profile/drafts/values удаляются, final-оценки блокируют удаление | `401`, `404`, `409` |
| `POST` | `/api/employees/{employee_id}/bot-link/reset` | Сбросить Telegram/runtime-привязку карточки для повторного тестового linking. | Path: `employee_id` | Employee detail payload | Очищает Telegram identity, active messenger accounts, menu state (`current_menu_set_id`, `current_menu_path`), незавершенный scenario progress где employee является context или recipient, и pending launch requests для карточки; completed progress сохраняется как history | `401`, `404` |
...
| Method | Path | Назначение | Основные inputs | Response | Side effects | Частые errors |
| --- | --- | --- | --- | --- | --- | --- |
| `GET` | `/api/flows/workspace` | Вернуть sidebar сценариев/опросов и payload выбранного item. | Optional query: `scenario_id`, `kind=scenario\|survey` | Workspace payload with `kind`, `item_label`, `scenarios[]`, optional `workspace`; step payload содержит `is_terminal`, graph node содержит `is_terminal` и `is_explicit_terminal` | Нет | `401` |
...
| `POST` | `/api/flows/workspace/scenarios/{scenario_id}/settings` | Обновить item-level metadata. | JSON: `description`, preferred `role_scopes[]`, legacy `role_scope`, `employee_scope`, `recipient_mode`, `trigger_mode`, `candidate_work_stage_trigger`, `target_employee_id`; пустой `role_scopes[]` или `role_scope=all` означает все должности; `candidate_work_stage_trigger` принимает canonical key или label HR-статуса и сохраняется как key только для `trigger_mode=candidate_hr_stage` | `{ message, payload }` | Обновляет одну строку `scenario_templates`; `role_scopes[]` сохраняется в `role_scope` как CSV slug'ов или `all`, `recipient_mode` управляет адресатом runtime-шага, а progress все равно остается на context employee | `401`, `404` |
...
| `POST` | `/api/flows/workspace/steps/{step_id}` | Обновить один workspace node. | JSON fields: `title`, `text`, `response_type`, `button_options`, `confirm_choice`, `send_mode`, `send_time`, `target_field`, `launch_scenario_key`, `return_to_step_key`, `is_terminal`, `send_employee_card`, `attachment_document_item_id`, `notify_on_send_text`, `notify_on_send_recipient_ids`, `notify_on_send_recipient_scope`, `step_send_notifications[]`, `button_notifications[]`; `target_field` допускает semantic value `test_task_result` для ответа на тестовое; text fields хранятся как `TEXT`, но runtime рендерит их как TelegramSafeHTML subset; для notification recipients supported tokens only: `hr`, `manager`, `mentor_adaptation`, `mentor_ipr` | `{ message, payload, step_id }` | Обновляет одну `flow_step_templates` строку и related notification fields; `confirm_choice=true` для `buttons/branching` включает Telegram edit-message подтверждение выбора до сохранения ответа; `is_terminal=true` у leaf-шага завершает сценарий после успешной отправки или валидного ответа, а у container-шага `branching/chain` сначала запускает выбранную дочернюю цепочку и завершает сценарий после нее без перехода к следующему root-шагу; `attachment_document_item_id` сохраняет ссылку на active `document_library_items` и имеет runtime priority над uploaded attachment; legacy `attachment_path` может быть локальным путем или HTTP(S)-ссылкой; новые сохранения нормализуют recipients в role-only contract, legacy `employee:{id}` допускается только как runtime fallback для старых DB rows | `401`, `404` |
...
| `POST` | `/api/bulk-actions/messages/send` | Немедленно отправить массовое сообщение. | JSON: `message_text` в TelegramSafeHTML subset, target fields, `confirmed=true` | `{ message, payload }` | Рендерит/санитизирует сообщение единым TelegramSafeHTML path, отправляет Telegram messages с HTML parse mode и пишет manual `mass_message_actions` row | `401`, `400` |
...
- Для candidates смена `candidate_work_stage` создает `status_transition` launch request только если новое значение реально отличается от прежнего и есть matching scenario с `trigger_mode=candidate_hr_stage`.
- Пустые date fields в employee update являются clear-семантикой: `first_workday: ""` и `first_workday: null` сохраняют `NULL`.
