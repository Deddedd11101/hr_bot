...
| `flow_step_templates` | Step definitions | `flow_key`, `step_key`, `parent_step_id`, `branch_option_index`, `response_type`, `button_options`, `return_to_step_key`, scheduling fields, target field, attachment fields, notification fields | Root steps имеют `parent_step_id = NULL`; branches и chains вложены через `parent_step_id`; `return_to_step_key` разрешает branch-step вернуться в root-step того же сценария |
...
| `scenario_progress` | Runtime position сценария по employee | `employee_id`, `scenario_key`, `current_step_key`, `step_history`, `response_undo_history`, `waiting_for_response`, `is_completed`, timestamps | Tracks active/completed scenario state; `step_history` хранит навигацию назад, `response_undo_history` — JSON-стек rollback snapshots для последнего подтвержденного ответа |
