...
| --- | --- | --- | --- |
| `employees` | Единая запись для candidates и employees | `full_name`, explicit `first_name`, `first_workday`, `employee_stage`, `candidate_work_stage`, legacy Telegram fields, HR profile fields, consent flags, `is_bot_blocked` | Центральная запись, на нее ссылается большинство runtime tables; `first_name` не берется из Telegram display name |
...
| `positions` | Управляемый справочник должностей | `title`, `slug`, `is_active`, `sort_order`, timestamps | Используется settings UI, employee forms, scenario role scope и targeting; стартовый каталог задается `CANONICAL_POSITION_TITLES`, а `employees.desired_position` остается строкой для backward compatibility |
...
| `bot_menu_sets` | Employee-facing bot menu groups | `title`, `description/menu_text`, optional `button_rows` JSON, `sort_order`, `employee_scope`, `role_scope`, explicit target fields, `system_tag` | Root set uses persistent reply keyboard; nested sets use editable inline message; `button_rows` stores owned button IDs by visual row, while missing/null preserves legacy `sort_order`; `menu_text` is rendered with safe HTML and employee tags |
...
| `telegram_custom_emojis` | Explicit Telegram custom emoji catalog | `title`, numeric `emoji_id`, ordinary `fallback`, `is_active`, timestamps | Text messages may use safe `tg-emoji`; inline/reply keyboard labels use fallback because Telegram button text has no message entities |
...
- `pending_confirmation_value` фиксирует неподтвержденный button/branching выбор и является source of truth при `Подтвердить`.
- `pending_confirmation_option_index` и `pending_confirmation_message_id` нужны только для best-effort notification lookup и Telegram edit-message UX.
- Это нужно, чтобы:
...
- отдельный новый recipient-sensitive участок schema patching:
  ...
  - `scenario_progress.pending_confirmation_value`;
  - `scenario_progress.pending_confirmation_option_index`;
  - `scenario_progress.pending_confirmation_message_id`;
