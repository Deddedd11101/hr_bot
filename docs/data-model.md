...
- Generic employee detail payload исключает semantic file categories `resume`, `test_result`, `test_explanation`, `offer_document` и document slots `resume`, `offer`, `test_task_result`, `test_task_explanation`, чтобы не смешивать продуктовые документы с общими файлами.
