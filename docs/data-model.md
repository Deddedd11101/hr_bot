...
| `flow_launch_requests` | Launch queue для manual и scheduled work | `employee_id`, `flow_key`, `requested_at`, `processed_at`, `processing_status`, `processing_attempts`, `claimed_at`, `completed_at`, `failed_at`, `last_error`, `launch_type`, `skip_step_key` | Используется для будущих запусков и follow-up continuation steps; scheduler claim-ит pending request conditional update до side effects, stale `processing` старше 15 минут переводит в terminal `failed`; SQLite guard добавляет indexes `(processing_status, requested_at)` и `(employee_id, flow_key, skip_step_key, processing_status)` |
