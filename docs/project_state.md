...
- Должности перестали быть только набором legacy-констант: добавлен code-backed каталог `positions` с CRUD API `/api/settings/positions*`, SQLite compatibility patching и seed стартовых значений. Employee detail, scenario `role_scope`, mass actions и bot-menu targeting теперь опираются на общий position resolver, а старые строковые `employees.desired_position` автоматически подхватываются в каталог и не теряются.
- Scenario settings backend теперь принимает несколько должностей через `role_scopes[]`: в storage это остается `scenario_templates.role_scope` как `all` или CSV slug'ов, workspace payload отдает и raw `role_scope`, и нормализованный массив, а runtime matching проходит, если должность сотрудника или кандидата входит в выбранный набор.
...
- Bot menu runtime получил явный navigation contract: `/start` возвращает пользователя в root menu set, если он доступен, но больше не отправляет отдельное техническое приветствие перед registration-сценарием или меню; если меню не настроено, бот больше не шлет системный fallback-текст про отсутствие меню. Подменю ведутся через `Employee.current_menu_path`, а системные кнопки `Назад` и `Главное меню` добавляются runtime-слоем и запрещены для пользовательских кнопок. На `/app/bot-menu` можно выбрать общий fallback root, отдельный root для сотрудников и отдельный root для кандидатов; рассылка главного меню открывает каждому привязанному пользователю его актуальный root по аудитории. Сам редактор меню перешел на drill-down модель: обзор показывает наборы как папки с root-badges и счетчиками связей, а конкретный набор открывается через `?set_id=` с навигацией по родителям, дочерним наборам и быстрым переходом для кнопок `open_set`.
- `/start` запускает первый подходящий сценарий с `trigger_mode=bot_registration` только при candidate create/link event: сценарий должен матчиться по `employee_scope=candidates`, а для `staff` / `adaptation` / `ipr` username-linking ограничивается привязкой numeric Telegram ID и показом employee menu без candidate registration. Повторный `/start` не перезапускает registration-сценарий и остается навигационным возвратом в root menu.
- HR lifecycle перестал упираться только в ручной запуск сценариев: candidate status shortlist нормализован вокруг реальных HR-этапов (`Наш отказ`, `Собеседование с HR`, `Собеседование с руководителем`, `Тестирование`, `Оффер`, `Преонбординг`, `Отказ кандидата`), а scenario metadata теперь умеет `trigger_mode=candidate_hr_stage` с explicit `candidate_work_stage_trigger`, который backend превращает в `status_transition` launch request при реальном изменении статуса в карточке кандидата. Worker обрабатывает такие requests как immediate triggers без зависимости от `first_workday`, отбрасывает stale requests после последующей смены статуса и матчится по нормализованным stage keys/labels.
...
- React employee detail получил отдельный operator send-path для точечного Telegram-сообщения: `POST /api/employees/{id}/bot-message` пишет read-only audit в `manual_bot_message_history`, не лезет в `scenario_progress` и не создает `flow_launch_requests`. Это правильно для ad-hoc HR-коммуникации, но пока еще не заменяет ни сценарные уведомления, ни массовые сообщения.
...
- Scenario notification recipients теперь поддерживают системный token `hr` как постоянный allowed editor/API token: delivery требует `HrSettings.telegram_user_id`, а если он не задан, runtime пропускает HR notification так же, как unresolved `manager` / `mentor_adaptation` / `mentor_ipr`, без заведения HR как фейкового сотрудника.
- Scenario formatter теперь имеет явный template-tag contract для UI: тексты шагов поддерживают `{employee_full_name}`, `{position}`, `{first_workday}`, а step/button notifications через тот же formatter дополнительно могут использовать `{resume}` как безопасное имя последнего файла категории `resume` без раскрытия локального storage path. Existing `{doc:...}` tags остались отдельной document-slot механикой.
...
- Role semantics в карточке сотрудника начали отделяться от lifecycle semantics: для сотрудников добавлены flags `is_manager` / `is_mentor`, manager/mentor selects больше не должны брать любого `staff`, а trigger `manager_assigned_adaptation` создает launch request у назначенного руководителя, не смешивая это с date-based scheduler flow.
- Для назначений руководителя и наставников появился отдельный audit-layer: `employee_assignment_history` хранит смену `manager`, `mentor_adaptation` и `mentor_ipr` без подмены текущих полей в `employees`, поэтому карточка остается source of truth для current state, а история — для разбора кто и когда был назначен.
...
- Per-button notifications в React workspace больше не ограничены одной записью на кнопку: у каждой кнопки теперь есть список notification rules с add/edit/delete через modal workflow. Notification recipient contract тоже ужат: новые payloads больше не сохраняют `employee:{id}` или сырые chat ids, а работают только с role tokens `hr`, `manager`, `mentor_adaptation`, `mentor_ipr`.
- React workspace теперь так же умеет редактировать и step-level notifications как список правил, а не как один плоский набор полей. Для этого добавлена отдельная сущность `StepSendNotification`: runtime отправляет все правила по порядку, copy/delete flows сценариев сохраняют эти правила, а legacy `notify_on_send_*` поля оставлены только как compatibility seam для first-rule fallback. Старые `employee:{id}` notification rows не выкинуты принудительно из БД, но остаются только runtime fallback и больше не должны генерироваться UI/API.
...
- React scenario workspace больше не открывает конструктор сразу на первом экране: `/app/flows/workspace-v2` работает как каталог сценариев, а редактор конкретного сценария открывается через `?scenario_id=<id>`. В редакторе убрана колонка со всеми сценариями, рабочая область состоит из списка/схемы шагов и detail-pane выбранного шага; настройки сценария доступны из каталога через menu/dialog карточки.
