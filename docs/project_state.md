...
- Scenario delivery reliability закрывает P1-риск дублей: `FlowLaunchRequest` получает lifecycle `pending -> processing -> processed|failed`, scheduler claim-ит request atomic conditional update до Telegram side effects, stale `processing` старше 15 минут переводит в terminal `failed`, а follow-up requests dedup'ятся по `employee_id + flow_key + skip_step_key`. Авто-retry failed request пока не включен намеренно, чтобы не повторять потенциально уже отправленный шаг; failed/stale видны в employee detail API как diagnostic history.
- Incoming text/file/back больше не выбирают newest active waiting progress при конфликте нескольких сценариев на одного recipient: runtime fail-closed, пишет `last_delivery_error` и просит пользователя обратиться к HR или перезапустить нужный сценарий.
