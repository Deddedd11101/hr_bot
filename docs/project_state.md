...
- В React scenario workspace восстановлен самостоятельный тип ответа `buttons`: редактор больше не маскирует его под `branching`, поэтому шаги с вариантами ответа снова могут сохранять `target_field` вроде `salary_expectation`, а runtime/API-контракт покрыт smoke-тестами.
