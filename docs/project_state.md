...
- Bulk-actions backend-slice тоже уже отделен на React/API уровне: targeting/workspace helpers вынесены в `app/web/bulk_actions.py`, а `/bulk-actions`, `/app/bulk-actions` и `/api/bulk-actions*` вынесены в `app/web/bulk_action_routes.py`. Classic bulk form handlers пока еще остаются в `app/main.py`.
- Settings backend-slice тоже уже отделен на React/API уровне: HR settings, menu sets/buttons и accounts workspace helpers вынесены в `app/web/settings.py`, а `/settings`, `/app/settings`, `/api/settings*` и `/api/accounts*` вынесены в `app/web/settings_routes.py`. Classic settings form handlers пока еще остаются в `app/main.py`.
- Scenario/surveys React/API workspace slice тоже уже вынесен: workspace helpers и step/template operations вынесены в `app/web/scenarios.py`, а `/flows`, `/surveys`, `/app/flows/workspace-v2`, `/app/surveys/workspace` и `/api/flows/workspace*` вынесены в `app/web/scenario_routes.py`. Classic editor routes `/flows/{id}`, `/surveys/{id}` и survey export пока еще остаются в `app/main.py`.
...
- `app/main.py` все еще ненормально большой для живого проекта: route handlers, auth/session, template rendering и API logic по-прежнему живут в одном файле. Phase 1 extraction (`app/web/support.py`) снял только самый безопасный слой; дальше нужна постепенная vertical decomposition, иначе любой следующий backend diff останется слишком широким.
- Даже после employee route split `app/main.py` все еще остается слишком большим: classic employee form handlers, bulk-actions, settings и scenario/survey routes по-прежнему склеены в одном модуле. Если начать удалять старые страницы раньше, чем эти срезы будут отделены, мы снова получим хаос, только уже backend-side.
- Даже после employee и bulk classic tail split `app/main.py` все еще остается слишком большим: classic settings form handlers и classic scenario/survey editor tails по-прежнему склеены в одном модуле. Если начать удалять старые страницы раньше, чем эти хвосты будут изолированы, мы снова получим хаос, только уже backend-side.
...
- Для backend-слоя ближайший structural шаг теперь тоже зафиксирован: после вынесения support-layer из `main.py` продолжать не массированным route rewrite, а vertical slices (`employees`, `bulk-actions`, `settings`, `scenario/surveys`) с обязательным green smoke после каждого выноса.
- Employee React/API routes уже вынесены; следующий backend slice теперь не `employees`, а `bulk-actions` или `settings`. Classic employee form handlers пока можно оставить на месте до отдельного parity/cleanup прохода.
- Employee React/API routes, classic employee tails и classic bulk tails уже вынесены; следующий backend slice теперь cleanup classic settings form handlers и classic scenario/survey editor tails.
