...
- Для admin UI принят обязательный shadcn workflow gate: перед изменением компонента или page composition нужно сверять `npx shadcn@latest info --json`, читать `npx shadcn@latest docs <component>` и локальный `frontend/src/components/ui/*`, а новые page patterns сначала фиксировать в `/app/design-system#patterns`.
...
- Вместо этого `/app/design-system` теперь закреплен как живой frontend baseline: страница должна документировать реальные shared primitives, page patterns и review rules, а не быть отдельной декоративной витриной. Auth form зафиксирован как pre-auth page pattern и применяется на `/login`.
...
- Редактор `/app/bot-menu` теперь также жестче отражает action contract самой кнопки: `launch_scenario`, `open_set` и `send_document` показываются как взаимоисключающие цели, а неактуальные селекты блокируются и очищаются при смене действия, чтобы оператор не собирал ложные комбинации.
...
- В runtime сценариев добавлен безопасный `Назад`: bot хранит короткую историю интерактивных шагов в `scenario_progress.step_history`, показывает default back-control внутри активного сценария и умеет возвращать пользователя на предыдущий text/file/buttons/branching step без отката уже выполненных side effects.
...
- Step/button notification rules перестали быть слепы к системному HR: сценарный workspace теперь отдает отдельного recipient-а `HR` из `HrSettings`, а runtime умеет резолвить token `hr` в основной HR Telegram ID без необходимости заводить HR как псевдо-сотрудника.
- Scenario runtime/editor получили отдельный interactive response type `date`: для шагов вроде оффера бот теперь может показывать inline-календарь вместо свободного текстового ввода, а выбранная дата может записываться напрямую в `first_workday`.
...
- Дополнительно выровнен старый drift между editor и runtime для `branching`: bot runtime и раньше умел сохранять выбранный вариант ветвления в поле карточки, а workspace API до этого насильно сбрасывал `target_field`. Теперь `branching` тоже может легально сохранять ответ, если это нужно сценарию.
- Scenario settings перестали быть привязаны к одной должности: React workspace теперь умеет выбирать несколько ролей сразу, backend хранит их как нормализованный набор в `scenario.role_scope`, а runtime матчит сотрудника по любому из выбранных role scopes вместо single-value сравнения.
...
- `employee_messenger_accounts.employee_id` пока не защищен foreign key/cascade, а SQLite foreign keys на stage выключены. Из-за этого удаление карточки может оставить orphan identity и блокировать повторную Telegram-привязку; runtime теперь безопасно освобождает только подтвержденные orphan accounts, но schema-level cleanup остается отдельной задачей.
- SMTP-слой для staff OTP теперь стал частью operational baseline бота: если `work_email` не заполнен или почтовые env-настройки невалидны, существующий сотрудник не сможет пройти первичную Telegram-привязку без ручного обхода через админку.
...
- Security/compliance слой пока не выделен отдельной реализацией: роли, аудит, защита файлов/персональных данных, секреты, backup policy, CSRF и broader hardening требуют отдельного прохода после стабилизации основных модулей.
- Auth/access сейчас реализован минимально, но первый session-hardening уже сделан в коде: два seeded account type `admin` и `hr`, signed stateless cookie вместо raw account id, базовый login rate limit и запрет слабых новых паролей. `admin` нужен только для управления аккаунтами, а большинство operator API доступны любой авторизованной роли. Целевая модель для HR/директора/технического администратора еще не выбрана.
