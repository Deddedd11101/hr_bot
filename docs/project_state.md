...
- Bot menu runtime получил явный navigation contract: `/start` всегда возвращает пользователя в root menu set, подменю ведутся через `Employee.current_menu_path`, а системные кнопки `Назад` и `Главное меню` добавляются runtime-слоем и запрещены для пользовательских кнопок. На `/app/bot-menu` можно выбрать главный набор и принудительно разослать его уже привязанным пользователям.
