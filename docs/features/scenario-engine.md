...
- Workspace и classic save отклоняют несовместимые настройки: `resume` / `candidate_file` требуют `response_type=file`, `test_task_result` допускает `file` или `text`. Старые конфигурации не переписываются при запуске приложения.
...
- Для трёх несовместимых шагов из диагностики 22.09 подготовлен `tools/repair_candidate_answer_steps.py`: по умолчанию read-only plan (`--db <db>`), применение только интегратором после deploy нового runtime (`--apply --backup <new-backup.db>`). Инструмент проверяет `flow_key`/`step_key` и старые значения всех трёх строк до любых записей, блокирует конкурентную запись на время ремонта, создаёт SQLite backup с `quick_check`, меняет только `target_field` на `test_task_result`. Повторный запуск не меняет исправленные строки. Изменившаяся конфигурация останавливает весь ремонт; запрещено обходить эту проверку без нового анализа. Source snapshot и live DB в разработке не изменялись. Loom-шаг дизайнера сознательно не входит в ремонт: второй ответ требует решения о хранении/показе нескольких результатов.
