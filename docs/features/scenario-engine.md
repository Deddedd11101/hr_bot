...
В тексте сообщений разрешен ограниченный HTML subset, включая numeric `<tg-emoji emoji-id="...">`. Telegram Bot API также поддерживает `icon_custom_emoji_id` у inline/reply-кнопок, но текущий редактор пока не связывает каталог с отдельными кнопками; до отдельного button-icon slice клавиатуры используют обычный emoji fallback. Custom emoji не переносится автоматически простым копированием из Premium-клиента.
