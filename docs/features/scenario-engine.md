...
- Шаг с `response_type=file` или `text` и `target_field=test_task_result` / `test_assignment_answer` означает “ответ на тестовое”: runtime принимает file-like media (`document`, `photo`, `video`, `video_note`) и сохраняет его как `employee_files.category=test_result` плюс актуальный `employee_document_links.slot_key=test_task_result`.
- Для такого же test-task шага текстовый ответ должен содержать одну `http://` или `https://` ссылку, в том числе с пояснением. В link-backed slot сохраняется только извлечённый URL, пояснение не становится отдельной HR-заметкой. Несколько ссылок, URL с credentials, некорректным host/port и текст без ссылки не двигают сценарий: бот просит один однозначный ответ. URL не скачивается сервером. `Назад` восстанавливает предыдущий slot.
- Workspace и classic save отклоняют несовместимые настройки: `resume` / `candidate_file` требуют `response_type=file`, `test_task_result` и `test_task_explanation` допускают `file` или `text`. Старые конфигурации не переписываются при запуске приложения.
- Cloud Telegram Bot API ограничивает скачивание файла 20 МБ. Известный размер больше лимита отклоняется до скачивания, ошибка Telegram `file is too big` также получает отдельную подсказку отправить меньший файл или ссылку для тестового. Ответ не сохраняется и шаг остаётся ожидающим. Остальные ошибки загрузки сообщают о неуспехе; неполный скачанный файл удаляется. Локальный Bot API server и скачивание больших файлов в этот выпуск не входят.
- `target_field=test_task_explanation` сохраняет дополнительное пояснение (Loom) отдельно от основного результата: slot `test_task_explanation`, file category `test_explanation`. Файл/видео/одна ссылка с пояснением принимаются одинаково. `Назад` откатывает только соответствующий slot. Карточка показывает оба ответа в блоке тестового, один не заменяет другой; внутри каждого slot последующий ответ обновляет актуальное значение.
- Для четырёх шагов из диагностики 22.09 подготовлен `tools/repair_candidate_answer_steps.py`: по умолчанию read-only plan (`--db <db>`), применение только интегратором после deploy нового runtime (`--apply --backup <new-backup.db>`). Инструмент проверяет `flow_key`/`step_key` и старые значения всех строк до любых записей, блокирует конкурентную запись на время ремонта, создаёт SQLite backup с `quick_check`, меняет только `target_field`: три основных ответа на `test_task_result`, Loom на `test_task_explanation`. Повторный запуск не меняет исправленные строки. Изменившаяся конфигурация останавливает весь ремонт; запрещено обходить эту проверку без нового анализа. Source snapshot и live DB в разработке не изменялись.
...
- Для совместимости старых шагов `attachment_path` может содержать локальный файл или `http://` / `https://` ссылку; URL отправляется текстовым сообщением, а не проверяется как путь на диске. Новые reusable материалы предпочтительно выбирать из document library.
