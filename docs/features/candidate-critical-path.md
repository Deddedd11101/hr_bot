...
Шаг с `response_type=file` или `text` и `target_field=test_task_result`. Одна HTTP(S)-ссылка может сопровождаться пояснением; несколько ссылок требуют уточнения, сценарий остаётся ожидающим. Большие файлы (свыше 20 МБ для cloud Bot API) требуют меньшего файла или ссылки, а не повторной отправки того же файла.
