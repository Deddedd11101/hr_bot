...
| `HRB-P1-09` | `done` | Управляемый справочник должностей | Добавлена таблица `positions` с SQLite compatibility patching и канонический seed из согласованного набора должностей. Employee detail берет options должностей из БД, settings API умеет CRUD по `/api/settings/positions*`, а scenario role scope, массовые действия и bot-menu targeting используют общий position resolver. `employees.desired_position` остается строкой для backward compatibility; новые значения больше не создают catalog rows автоматически. Для существующего drift добавлены read-only audit и отдельный dry-run/backup-backed replacement tool. |
