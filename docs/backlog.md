...
Для пакетов дополнительно фиксировать независимые состояния «код проверен», «выложено», «пользовательский сценарий проверен» с SHA и доказательством. `done` ниже описывает реализацию и документацию, а не автоматически приемку на stage. Пилот координации: гибридное меню Telegram; Product сверяет PR и зависимости, deploy остается интегратору. Политика в AGENTS.md и [[stage-deploy]].
...
| --- | --- | --- | --- |
| `HRB-P0-01` | `done` | Управляемый `/start` для неизвестного Telegram-пользователя | Обычные text/file/callback события от неизвестного пользователя по-прежнему не создают runtime-данные, но `/start` теперь является осознанной candidate-entry точкой: бот сначала матчится по numeric Telegram ID, затем по public username, при отсутствии match создает candidate-карточку и запускает candidate `bot_registration` только один раз. |
...
| Token | Статус | Название | Описание |
| --- | --- | --- | --- |
| `HRB-P1-01` | `doing` | Запуск сценария по смене HR-статуса | Employee detail умеет запускать сценарии от реальной смены `candidate_work_stage`: у сценария есть `trigger_mode=candidate_hr_stage` и explicit `candidate_work_stage_trigger`, backend ставит `status_transition` launch request, worker обрабатывает его как immediate trigger без зависимости от `first_workday`, перед запуском отбрасывает stale request после последующей смены статуса и запускает обычный `start_scenario(...)`. Backend нормализует ключи и labels HR-статусов при сохранении/matching trigger settings; повторное сохранение того же статуса не ставит новый pending request. Дополнительно введен recipient-layer для сценариев: launch/progress остаются на context employee, а runtime может отправлять шаги руководителю, наставнику или HR через `recipient_mode`, что закрывает базовый seam для `manager_assigned_adaptation`. Следующий шаг — richer dedupe/cooldown policy и решить, нужен ли отдельный event/status для `offer accepted`, вместо слепого auto-convert из Telegram ответа. |
| `HRB-P1-02` | `doing` | Нормальная семантика перехода к другому сценарию | React workspace уже ушел от однослотовой notification-схемы и для кнопок, и для самих шагов: у обоих появились множественные rules через modal editor, а recipient contract нормализован до role-only tokens `hr/manager/mentor_adaptation/mentor_ipr` вместо explicit employee ids. Параллельно survey flow перестал маскироваться под полноценный сценарий: question flow теперь текстовый с optional answer variants, без branching semantics. Runtime-переход `launch_scenario` для обычного шага больше не зависает молча: после отправки шага сценарий корректно завершается и стартует target scenario. Дополнительно branch-step теперь умеет вернуться в root-step того же сценария через `return_to_step_key`, чтобы не дожимать весь общий flow внутри chain-веток. Следующий шаг — cleanup remaining classic editor semantics и явное product-правило для пустого target scenario. |
| `HRB-P1-03` | `doing` | Унификация уведомлений | Notification model начала выравниваться: button notifications уже живут в `StepButtonNotification`, step-level notifications переведены в `StepSendNotification`, runtime отправляет все rules по порядку, а новые scenario notification saves теперь ограничены role-only recipient contract `hr/manager/mentor_adaptation/mentor_ipr`. Legacy `employee:{id}` оставлен только как runtime compatibility seam для уже существующих DB rows. Оставшийся шаг — убрать product ambiguity между сценарными уведомлениями и глобальными system notifications/delivery rules. |
...
| `HRB-P1-09` | `done` | Управляемый справочник должностей | Добавлена таблица `positions` с SQLite compatibility patching и seed стартовых ролей. Employee detail теперь берет options должностей из БД, settings API умеет CRUD по `/api/settings/positions*`, а scenario role scope, массовые действия и bot-menu targeting больше не обязаны жить только на захардкоженных `designer/project_manager/analyst`. Backward compatibility сохранена: `employees.desired_position` пока остается строкой, legacy значения автоматически попадают в каталог и продолжают матчиться со сценариями и таргетингом. |
...
`HRB-DISC-06`: scope первого выпуска [[features/archive-reentry-proposal|архива и повторного прихода]] согласован; подготовлены [[features/archive-reentry-tasks|задачи и критерии приёмки]]. Политика удаления остаётся открытой и не входит в выпуск.
...
| `HRB-DISC-06` | `doing` | Архив и повторный приход кандидата | Первый выпуск согласован и декомпозирован; технические детали BE-01 и будущая политика удаления/12 месяцев остаются открытыми. См. [[features/archive-reentry-proposal]], [[features/archive-reentry-tasks]]. |

### Первый выпуск архива

| Token | Статус | Задача | Зависимости |
| --- | --- | --- | --- |
| `HRB-ARCH-BE-01` | `todo` | Модель, права и API-контракт; независимый запрет доступа | Согласованный scope PR #11; DB/Storage review перед следующими задачами |
| `HRB-ARCH-BE-02` | `todo` | Архив, отмена доставок и восстановление | BE-01 |
| `HRB-ARCH-BE-03` | `todo` | Повторный вход, связь истории и назначения для замены | BE-01, BE-02 |
| `HRB-ARCH-FE-01` | `todo` | Интерфейс архива, восстановления, запрета и назначений | Контракт BE-01; приёмка после BE-02/BE-03 |
| `HRB-ARCH-QA-01` | `todo` | Общая техническая приёмка | BE-02, BE-03, FE-01, DB/Storage + Docs review |

Критерии приёмки: [[features/archive-reentry-tasks]]. Автоматическое удаление данных исключено из всех задач первого выпуска.
