...
| Token | Статус | Название | Описание |
| --- | --- | --- | --- |
| `HRB-P1-01` | `doing` | Запуск сценария по смене HR-статуса | Employee detail уже получил явный HR cutover `candidate -> adaptation` через отдельный API/action. Дополнительно введен первый event-driven assignment trigger: при назначении руководителя сотруднику в `adaptation` backend создает trigger launch request для manager-сценария `manager_assigned_adaptation`, а роль руководителя/наставника хранится как flags `is_manager` / `is_mentor`, а не как lifecycle status. Следующий шаг — решить, какие еще lifecycle/assignment events должны запускать сценарии, и нужен ли отдельный status/event для `offer accepted`, вместо слепого auto-convert из Telegram ответа. |
