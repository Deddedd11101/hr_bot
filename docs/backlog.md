...
| Token | Статус | Название | Описание |
| --- | --- | --- | --- |
| `HRB-P1-01` | `doing` | Запуск сценария по смене HR-статуса | Employee detail теперь умеет запускать сценарии от реальной смены `candidate_work_stage`: у сценария есть `trigger_mode=candidate_hr_stage` и explicit `candidate_work_stage_trigger`, backend ставит `status_transition` launch request и worker запускает обычный `start_scenario(...)`. Дополнительно введен recipient-layer для сценариев: launch/progress остаются на context employee, а runtime может отправлять шаги руководителю, наставнику или HR через `recipient_mode`, что закрывает базовый seam для `manager_assigned_adaptation`. Следующий шаг — закрыть полную trigger matrix, richer dedupe/cooldown policy и решить, нужен ли отдельный event/status для `offer accepted`, вместо слепого auto-convert из Telegram ответа. |
