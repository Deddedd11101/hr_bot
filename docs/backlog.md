...
| `HRB-P2-05` | `doing` | UX-функции конструктора | Базовый `шаг назад` в runtime бота уже добавлен через `scenario_progress.step_history` и default back-controls. В React scenario workspace начат editor-side guardrail: тип ответа теперь явно помечает шаги, которые блокируют поток и ждут ответ пользователя; для `launch_scenario` disabled downstream select больше не притворяется активным при другом типе ответа. Оставшийся scope: более явная визуализация flow и, если понадобится, настоящий undo для terminal/side-effect branches. |
