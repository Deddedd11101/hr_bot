...
| `HRB-P2-04` | `doing` | Календарь и типизированные ответы в боте | Для scenario steps добавлен `response_type=date`: runtime показывает inline-календарь, workspace editor разрешает выбрать этот тип и сохранить результат в `first_workday`. Scheduler для time-based сценариев догоняет первый непройденный шаг в день активации, если его время уже прошло, и не дублирует следующий `specific_time` шаг через internal follow-up. Оставшийся scope: расширить date-target fields при реальной потребности и добавить более богатую валидацию дат. |
