...
`HRB-DISC-06`: подготовлен [[features/archive-reentry-proposal|проект правил архива и повторного прихода]]. Обсуждение открыто; реализация и очистка данных не разрешены этим документом.
...
| `HRB-DISC-06` | `doing` | Архив и повторный приход кандидата | Согласовать первый выпуск: архив/восстановление, отмена доставок, назначения для замены, повторный вход и отдельный запрет, связь попыток. После согласования подготовить backend/frontend-задачи. Политика удаления и 12 месяцев остаются отдельным нерешённым этапом без автоматической очистки. См. [[features/archive-reentry-proposal]]. |
