...
class ScenarioPortabilityTests(unittest.TestCase):
    ...
    def test_export_import_round_trip_preserves_confirm_choice(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            source_db = root / "source.db"
            target_db = root / "target.db"
            ...
            source = sqlite3.connect(str(source_db))
            try:
                source.execute("ALTER TABLE flow_step_templates ADD COLUMN confirm_choice INTEGER NOT NULL DEFAULT 0")
                source.execute(
                    """
                    INSERT INTO scenario_templates (
                        scenario_key, title, sort_order, scenario_kind, role_scope, employee_scope, recipient_mode, trigger_mode, target_employee_id, description
                ...
                source.execute(
                    ...
                    INSERT INTO flow_step_templates (
                        ...
                        send_employee_card, notify_on_send_text, notify_on_send_recipient_ids, notify_on_send_recipient_scope
                    ) VALUES ('scenario_confirm', 'salary_step', NULL, NULL, 'Salary', 10, 'Pick salary', NULL, 'buttons', '200000\n300000', 1, 'immediate', NULL, 0, 'salary_expectation', NULL, NULL, NULL, 0, NULL, NULL, NULL)
                    """
                )
                source.commit()
            finally:
            ...
            try:
                row = target.execute(
                    """
                    SELECT confirm_choice
                    FROM flow_step_templates
                    WHERE flow_key = 'scenario_confirm' AND step_key = 'salary_step'
                    """
                ).fetchone()
            finally:
                target.close()
    ...
    def test_export_from_old_db_without_attachment_document_column_does_not_fail(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIsNone(step["return_to_step_key"])
            ...
            self.assertNotIn("return_to_step_key", columns)
